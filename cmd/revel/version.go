...
func (v *VersionCommand) doRepoCheck(updateLibs bool) (versionInfo string, needsUpdate bool) {
	...
	for _, repo := range []string{"revel", "cmd", "modules"} {
		...
		switch repo {
		case "revel":
			title, repo, localVersion = "Revel Framework", "github.com/caeril/frevel/revel", v.revelVersion
		case "cmd":
			title, repo, localVersion = "Revel Cmd", "github.com/caeril/frevel/cmd/revel", v.cmdVersion
		case "modules":
			title, repo, localVersion = "Revel Modules", "github.com/caeril/frevel/modules", v.modulesVersion
		}
